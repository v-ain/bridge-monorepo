...
async function apiRequest_V2<T>(endpoint: string, options: RequestInit = {}): Promise<ApiResponse<T>> {
...
export const notesApi_V2 = {
  getAll: () => apiRequest_V2<NoteEntity[]>('/notes'),

  getById: (id: string) => apiRequest_V2<NoteEntity>(`/notes/${id}`),

  create: (title: string) => apiRequest_V2<NoteEntity>('/notes', {
    method: 'POST',
    body: JSON.stringify({ title })
  }),

  update: (id: string, title: string) => apiRequest_V2<NoteEntity>(`/notes/${id}`, {
    method: 'PATCH',
    body: JSON.stringify({ title })
  }),

  delete: (id: string) => apiRequest_V2<{ id: string }>(`/notes/${id}`, {
    method: 'DELETE'
  }),
};
