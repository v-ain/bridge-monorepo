...
import { NoteEditForm } from './NoteEditForm_v2';
...
export const NoteItem = ({ note, onModal }: NoteItemProps) => {
  ...
  const updateNote = useNoteStore_v2((state) => state.updateNote);
  const deleteNote = useNoteStore_v2((state) => state.deleteNote);
  const toggleEditMode = useNoteStore_v2((state) => state.toggleEditMode);
  const toggleSelect = useNoteStore_v2((state) => state.toggleSelect);
