...
echo -n "  POST /api/notes (valid)..."
TEST_FILE="test_note.txt"
echo '{"title": "Текст с \"кавычками\""}' >$TEST_FILE
...
echo -n "  POST /api/notes (2MB payload)..."
...
echo -n "  GET /api/notes/99999 (non-existent)..."
...
echo -n "  GET /api/notes/some-random-id (not found)..."
...
echo -n "  DELETE /api/notes/:id (existing)..."
...
echo -n "  DELETE /api/notes/99999 (non-existent)..."
...
echo -n "  DELETE /api/notes/abc (not found)..."
...
echo -n "  PATCH /api/notes/:id (update content)..."

# Создаем заметку
RESPONSE=$(curl -s -X POST $NOTES_URL -H "Content-Type: application/json" -d '{"title": "original"}')
ID=$(echo $RESPONSE | grep -oE '"id":"([^"]+)"' | cut -d'"' -f4)
